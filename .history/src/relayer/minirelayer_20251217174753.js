...
class MiniMidenClient {
  ...
  async submitBridgeProof(obj) {
    // Simulate Miden enforcing nullifier uniqueness
    const nullifier = obj && obj.nullifier;
    if (!nullifier) return { ok: false, error: 'missing_nullifier' };
    if (this.nullifierSet.has(nullifier)) {
      return { ok: false, error: 'nullifier_used' };
    }
