...
class MiniMidenClient {
  constructor() {
    // Instead of keeping nullifiers here, delegate to the simulated Miden program
    this.program = new SimMidenBridge();
  }

  async submitBridgeProof(obj) {
    if (!obj || !obj.nullifier) return { ok: false, error: 'missing_nullifier' };
    // call the simulated program which will verify proof and enforce nullifiers
    return await this.program.verifyAndApply(obj);
  }
}
...
export default class MiniRelayer extends EventEmitter {
  ...
  async extractBridgeData(tx) {
    for (const out of tx.vout || []) {
      ...
      if (parts.length >= 3 && parts[0] === 'OP_RETURN' && parts[1] === 'BRIDGE') {
        ...
        if (envelopeB64) {
          try {
            const { decryptEnvelope } = await import('./envelope.js');
            const env = decryptEnvelope(envelopeB64);
            if (env) {
  ...
  async processBridgeTransaction(tx, height) {
    ...
    // Only record nullifier locally if Miden accepted the proof (simulating on-chain enforcement)
    if (res && res.ok) {
