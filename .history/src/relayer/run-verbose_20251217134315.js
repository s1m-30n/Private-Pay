...
// Verbose runner for local relayer simulation that forces a short poll interval
// and prints the persisted .relayer_state.json after processing.
...
async function runVerbose() {
  console.log('Starting verbose relayer run...');

  // Minimal config override: fast poll interval and simulation clients
  const cfg = {
    useRealClients: false,
    pollInterval: 200, // 200ms poll to speed up test
    zcash: {},
    miden: {},
    zcash: { minConfirmations: 0 }
  };

  const relayer = new ZcashMidenRelayer(cfg);

  // Start relayer
  await relayer.start().catch(err => console.error('Relayer failed to start', err));

  // Wait briefly so relayer has initialized its mock client
  await new Promise(r => setTimeout(r, 100));

  // Create and push a mock bridge tx
  ...
  // Wait up to 5 seconds for the relayer to process the block
  ...
  // Read and print state file if it exists
  try {
    const raw = await fs.readFile(STATE_FILE, 'utf8');
    console.log('\n.relayer_state.json contents:\n');
    console.log(raw);
  } catch (err) {
    console.error('\n.relayer_state.json not found or could not be read:', err.message);
  }
}

if (import.meta.url === `file://${process.cwd()}/src/relayer/run-verbose.js`) {
  runVerbose().catch(err => {
    console.error('run-verbose encountered an error:', err);
    process.exit(1);
  });
}
// src/relayer/run-verbose.js
